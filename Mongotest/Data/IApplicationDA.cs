...
namespace Mongotest.Data
{
    public interface IApplicationDA
    {
        ...
        Task DeleteAsync<T>(Guid id, string collectionName) where T : BaseModel;
        ...
        Task<T> GetOneAsync<T>(Guid id, string collectionName) where T : BaseModel;
        Task<T> UpsertAsync<T>(Guid id, T model, string collectionName) where T : BaseModel;
    }
}